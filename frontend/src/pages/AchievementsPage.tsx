...
export default function AchievementsPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <header>
        <div className="flex items-center gap-2"><div className="rounded-xl bg-lavender-600 p-2 text-white"><Trophy size={20} /></div><h1 className="text-2xl font-semibold text-slate-800">Achievements</h1></div>
        ...
        <section className="rounded-2xl border border-cream-200 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-lavender-600 text-xl font-bold text-white">{summary.level}</div>
              <div>
            ...
            <div className="w-full max-w-xs sm:w-56">
              ...
              <div className="mt-1 h-2 overflow-hidden rounded-full bg-cream-100"><div className="h-full bg-lavender-500 transition-all" style={{ width: `${summary.levelProgressPercent}%` }} /></div>
            </div>
          </div>
        </section>
      )}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {summary?.achievements.map((a) => {
          ...
          return (
            <article key={a.id} className={`rounded-2xl border p-5 shadow-sm ${a.unlocked ? "border-lavender-200 bg-lavender-50" : "border-cream-200 bg-white"}`}>
              <div className="flex items-start justify-between">
                <div className={`flex h-11 w-11 items-center justify-center rounded-xl ${a.unlocked ? "bg-lavender-600 text-white" : "bg-cream-100 text-slate-400"}`}>
                  {a.unlocked ? <Icon size={20} /> : <Lock size={18} />}
                </div>
                <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${a.unlocked ? "bg-lavender-600 text-white" : "bg-cream-100 text-slate-400"}`}>+{a.xp} XP</span>
              </div>
              ...
              <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-cream-100"><div className={`h-full ${a.unlocked ? "bg-lavender-500" : "bg-slate-300"}`} style={{ width: `${Math.round((a.current / a.target) * 100)}%` }} /></div>
