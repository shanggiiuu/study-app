...
export default function AiAdvice() {
  ...
  return <div className="flex flex-col rounded-2xl border border-cream-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><div className="flex h-7 w-7 items-center justify-center rounded-lg bg-brand-600 text-white"><Sparkles size={14} /></div><h2 className="text-base font-semibold text-slate-800">AI Advice For You</h2></div><div className="mt-4 flex-1 rounded-xl bg-brand-50 p-4">{loading ? <p className="text-sm text-slate-400">Preparing your personalized advice…</p> : error ? <p className="text-sm text-slate-500">{error}</p> : <p className="whitespace-pre-wrap text-sm leading-6 text-slate-700">{advice}</p>}</div><Link to="/advice" className="mt-4 flex items-center gap-1 text-sm font-semibold text-brand-600 hover:text-brand-700">Ask Advice AI <ArrowRight size={15} /></Link></div>;
}
